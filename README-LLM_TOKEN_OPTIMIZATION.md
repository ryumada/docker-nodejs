...
| `test-log-compression.md` | Merged into `docker-nodejs-management` skill |
| `run-npm-command.md` | Merged into `docker-nodejs-management` skill |
| `always-create-tests.md` | Removed — redundant / on-demand pattern |
...
## Summary: How the Layers Stack

```
Session Start
│
├── Layer 1: Slim always-on rules loaded. Dynamic ICM context injection (< 3 items).
│
├── Agent discovers files & symbols
│   ├── Layer 4: Reads compact REPO_MAP.md for file existence
│   └── Layer 4: Runs query_graph.py to trace AST callers/callees (< 20 lines)
│
├── Agent inspects code
│   └── Layer 5: Slice-reading (StartLine/EndLine) prevents reading 500+ line files
│
├── Agent edits code
│   ├── Layer 1: model_decision rules load (only what's needed)
│   └── Layer 2: Ponytail + Cavecrew keep diff output minimal
│
